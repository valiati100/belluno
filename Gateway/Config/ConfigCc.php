...
class ConfigCc extends Config
{
    ...
    {
        $data = [];
        $months = (new DataBundle())->get($this->_localeResolver->getLocale())["calendar"]["gregorian"]["monthNames"]["format"]["wide"];
    ...
    public function getIcons()
    {
        ...
        foreach($types as $code => $label) {
            if(!array_key_exists($code, $this->_icons)) {
                $asset = $this->_ccConfig->createAsset(
                    "Belluno_Magento2::images/cc/" . strtolower($label) . ".png"
                );
